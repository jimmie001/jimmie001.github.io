...
function showTask() {
  ...
  if (task === "poem") {
    ...
    content.innerText = "歷史錯亂如裂縫，＿＿";
    choices.innerHTML = `
      <button onclick="correct()">詩與真理為鑰</button>
      <button onclick="wrong()">虛幻埋葬真相</button>
    `;
  } else {
    title.innerText = "歷史任務";
    content.innerText = "歷史修正的關鍵手段？";
    choices.innerHTML = `
      <button onclick="correct()">恢復時代真實</button>
      <button onclick="wrong()">強化虛構敘事</button>
    `;
  }
  message.innerText = "";
}
